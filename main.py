...
from functools import wraps # Für den Authentifizierungs-Decorator
...
# Den Header holen wir uns jetzt hier, aber rendern ihn nur auf der Hauptseite
# nachdem die Authentifizierung geprüft wurde.
# Das globale Styling wird beim ersten Aufruf von apply_design_and_get_header() angewendet.
...
# Decorator für geschützte Seiten
def protected_page(func):
    @wraps(func)
    async def wrapper(client: Client, *args, **kwargs):
        if not get_current_user_id():
            # ui.open('/login') # Das funktioniert hier nicht zuverlässig vor dem ersten Page-Build.
            # Besser: Inhalt nicht rendern und eine Nachricht zeigen oder clientseitigen Redirect anstoßen.
            # Für eine robuste Lösung wird eine Weiterleitung oft clientseitig oder über app.on_connect gehandhabt.
            # Hier, für `@ui.page` Funktionen, prüfen wir am Anfang der Funktion.
            # Der Aufruf von ui.open in der Funktion selbst ist der übliche Weg.
            # print("DEBUG: User not authenticated, redirecting to /login")
            # await client.connected() # Ensure client is connected before ui.open
            # ui.open('/login')
            # return # Verhindert das Ausführen des restlichen Codes der Seite
            # Stattdessen wird die Prüfung in der Page-Funktion selbst durchgeführt.
            # Diese Decorator-Struktur kann nützlich sein, wenn man z.B. vorab Rollen prüfen will.
            # Für reines Login/Logout ist die Prüfung in der Funktion oft klarer.
            pass # Die Prüfung erfolgt jetzt direkt in der Seitenfunktion
        return await func(client, *args, **kwargs)
    return wrapper


async def init_user_specific_app_storage(client: Client):
    """Initialisiert oder setzt den benutzerspezifischen Speicher zurück."""
    await client.connected()
    user_id = get_current_user_id()
    if not user_id:
        print("WARNING: init_user_specific_app_storage called without logged-in user.")
        return
    ...
    # Initiales Laden der Tracks nach kurzer Verzögerung, nur wenn UI-Elemente schon da sind
...
async def initial_load_and_map_setup():
    ...
    if map_view:
        ...
        print("WARNING: map_view not found for initial_load_and_map_setup.")
...
async def login_page(client: Client):
    ...
    async def handle_login():
        ...
        try:
            user = db_config.get_user_by_username(db, username_input.value)
            if user and db_config.verify_password(password_input.value, user.hashed_password):
                ...
                await init_user_specific_app_storage(client) # User-spezifischen Storage initialisieren
                ui.navigate.to('/') 
            else:
    ...
    with ui.column().classes('absolute-center items-center gap-4 w-full max-w-xs p-8 rounded shadow-lg bg-white'):
        ...
        username_input = ui.input('Benutzername').props('outlined dense clearable').classes('w-full')
        password_input = ui.input('Passwort', password=True).props('outlined dense clearable password-toggle').classes('w-full')
        ui.button('Login', on_click=handle_login).props('color=primary unelevated').classes('w-full')
...
async def register_page(client: Client):
    if get_current_user_id(): # Wenn schon eingeloggt
        ui.navigate.to('/')
        return

    async def handle_register():
        if not username_input.value or \
           not email_input.value or \
           not password_input.value or \
           not password_confirm_input.value:
            ui.notify('Alle Felder sind Pflichtfelder.', type='warning')
            return
        ...
        db = db_config.SessionLocal()
        try:
            existing_user = db_config.get_user_by_username(db, username_input.value)
            if existing_user:
                ui.notify('Benutzername bereits vergeben.', type='negative')
                return
            
            # Optional: E-Mail validieren, wenn verwendet
            # if email_input.value and db_config.get_user_by_email(db, email_input.value):
            #     ui.notify('E-Mail bereits registriert.', type='negative')
            #     return

            db_config.create_user(db, username_input.value, password_input.value, email_input.value or None)
            ui.notify('Registrierung erfolgreich! Sie können sich nun einloggen.', type='positive')
            ui.navigate.to('/login')
        except Exception as e:
            print(f"Registrierungsfehler: {e}")
            traceback.print_exc()
            ui.notify('Registrierung fehlgeschlagen. Bitte versuchen Sie es später erneut.', type='negative')
        finally:
            db.close()

    with ui.column().classes('absolute-center items-center gap-4 w-full max-w-xs p-8 rounded shadow-lg bg-white'):
        ui.label('Registrieren').classes('text-2xl font-semibold text-primary')
        username_input = ui.input('Benutzername').props('outlined dense clearable required').classes('w-full')
        email_input = ui.input('E-Mail').props('outlined dense clearable type=email required').classes('w-full')
        password_input = ui.input('Passwort', password=True).props('outlined dense required').classes('w-full')
        password_confirm_input = ui.input('Passwort bestätigen', password=True).props('outlined dense required').classes('w-full')
...
async def main_page(client: Client):
    ...
    if not user_id:
        ui.navigate.to('/login')
        return # Wichtig: Verhindert das Rendern des Rests der Seite

    # Header dynamisch rendern (jetzt, wo wir wissen, dass der User eingeloggt ist)
    dynamic_header_renderer() # Ruft app_header() aus design.py auf

    # Initialisiere den User-spezifischen Teil von app.storage, wenn er zum ersten Mal auf die Seite kommt
    # oder wenn die Seite neu geladen wird und der User noch eingeloggt ist.
    if 'tracks_in_table_data' not in app.storage.user: # Nur initialisieren, wenn nicht vorhanden
         await init_user_specific_app_storage(client)
    
    # UI Aufbau (wie zuvor, aber mit user_id bei DB-Operationen)
    with ui.column().classes('w-full p-4 items-center gap-4'):
            ...
            with ui.card().classes('w-full md:w-1/2 lg:w-1/3 shadow-lg'):
                ...
                with ui.card_section():
                    ui.upload(label='GPX-Datei(en) auswählen oder hierhin ziehen',
                               on_upload=lambda e: handle_gpx_upload(user_id, e), # user_id übergeben
                ...
                with ui.card_section(), ui.column().classes('gap-2'):
                    with ui.row().classes('w-full items-center gap-2'):
                        date_from_input = ui.date(
                            ...
                            on_change=lambda e: update_filter_settings(user_id, 'date_from', e.value) # user_id
                        ...
                        date_to_input = ui.date(
                            ...
                            on_change=lambda e: update_filter_settings(user_id, 'date_to', e.value) # user_id
                    ...
                    label_select_ui = ui.select(
                        ...
                        on_change=lambda e: update_filter_settings(user_id, 'labels', e.value) # user_id
                    ...
                    ui.button('Filter zurücksetzen', icon='restart_alt',
                        ...
                        .props('flat dense color=grey-7').classes('mt-2 self-start') # user_id
            ...
            with splitter.after, ui.column().classes('w-full h-full'):
                with ui.card().classes('w-full h-full flex flex-col'):
                    with ui.card_section():
                        with ui.row().classes('w-full justify-between items-center'):
                            ...
                            delete_selected_button_ui = ui.button(icon='delete_sweep',
                                                                 on_click=lambda: confirm_delete_selected_tracks(user_id), # user_id
                                                                 color='negative') \
                    ...
                    track_table_ui = ui.table(columns=columns_def,
                                           ...
                                           on_select=lambda e: handle_table_selection_change(user_id, e), # user_id
                    ...
                    track_table_ui.on('editTrack', lambda e: open_track_edit_dialog(user_id, e.args['id'])) # user_id
                    track_table_ui.on('deleteTrack', lambda e: confirm_delete_single_track(user_id, e.args)) # user_id
        ...
        with ui.dialog().props('persistent') as local_dialog_ref, ui.card().style('min-width: 350px'):
            edit_dialog_instance = local_dialog_ref # global
            with ui.card_section(): ui.label('Track Bearbeiten').classes('text-h6')
            ui.separator()
            name_input_for_dialog = ui.input('Name').props('outlined dense').classes('w-full') # global
            labels_input_for_dialog = ui.input('Labels (Komma-getrennt)', placeholder="z.B. Alpen, Radtour") \
                .props('outlined dense').classes('w-full my-2') # global
            with ui.card_actions().props('align=right'):
                ui.button('Abbrechen', on_click=edit_dialog_instance.close).props('flat color=grey')
                ui.button('Speichern', on_click=lambda: save_edited_track_details(user_id)).props('color=primary') # user_id

    # UI Elemente im Client Storage für schnellen Zugriff speichern
    ...
    # Rufe update_all_db_labels_options_ui einmalig nach dem Aufbau der UI-Elemente auf,
    # um sicherzustellen, dass der Label-Filter initial gefüllt ist, falls schon Tracks da sind.
    # Dies wird auch von load_tracks_from_db_and_refresh_ui gemacht, aber hier explizit
    # für den Fall, dass die Tabelle leer ist, aber der Filter trotzdem Optionen haben sollte.
    # Ein erneuter Aufruf durch initial_load_and_map_setup ist nicht schädlich.
...
def format_track_for_display(track_db_obj: db_config.TrackDB) -> Dict[str, Any]:
    ...
    return {
        ...
        'stored_filename': track_db_obj.stored_filename, # Wichtig für Debugging, aber nicht direkt in Tabelle
        'total_ascent': track_db_obj.gpx_parsed_total_ascent, # Für Stats
    }
async def handle_gpx_upload(user_id: int, e: Any):
    ...
    try:
        ...
        print(f"ERROR during handle_gpx_upload for user {user_id}: {ex_upload}")
        traceback.print_exc()
        ui.notify(f"Schwerer Fehler beim Upload: {ex_upload}", type='negative', multi_line=True)
    finally:
        db.close()

# format_track_for_display bleibt gleich
...
async def load_tracks_from_db_and_refresh_ui(user_id: int, is_initial_load: bool = False):
    ...
    try:
        ...
        tracks_from_db = db_config.get_filtered_tracks(db, user_id, date_from, date_to, labels) # user_id
        ...
        if track_table:
            ...
            print(f"WARNING: ui_track_table not in client storage for update (user {user_id}).")

        update_all_db_labels_options_ui(user_id, db_session=db) # user_id
        await update_map_and_related_stats(user_id, is_initial_map_fit=(is_initial_load or app.storage.user.get('map_needs_initial_fit', False))) # user_id
        if is_initial_load or app.storage.user.get('map_needs_initial_fit', False):
            app.storage.user['map_needs_initial_fit'] = False
    except Exception as e_load:
        print(f"ERROR in load_tracks_from_db_and_refresh_ui for user {user_id}: {e_load}")
        traceback.print_exc()
...
def update_all_db_labels_options_ui(user_id: int, db_session: Optional[db_config.Session] = None):
    ...
    try:
        new_labels = db_config.get_all_unique_labels(db_to_use, user_id) # user_id
...
async def handle_table_selection_change(user_id: int, e: Any):
    ...
    await update_map_and_related_stats(user_id, is_initial_map_fit=False) # user_id
...
async def update_map_and_related_stats(user_id: int, is_initial_map_fit: bool = False):
    ...
    try:
        for track_data in selected_track_display_data:
            ...
            gpx_file_path = db_config.get_gpx_filepath(db, user_id, track_data['id']) # user_id
            if gpx_file_path and gpx_file_path.exists():
                ...
                print(f"WARNING: GPX file path not found for track ID {track_data['id']} (User {user_id}).")
    finally:
        db.close()
    ...
    if len(selected_track_display_data) == 1 and chart_container:
        ...
        try:
            gpx_file_path_chart = db_config.get_gpx_filepath(db_chart, user_id, track_for_profile['id']) # user_id
...
def open_track_edit_dialog(user_id: int, track_id: int):
    ...
    db = db_config.SessionLocal()
    try:
        track_to_edit = db_config.get_track_details(db, user_id, track_id) # user_id
        if track_to_edit:
            current_editing_track_id = track_id # Global, aber wird nur im Kontext des Users verwendet
            ...
            ui.notify(f"Track ID {track_id} nicht gefunden oder gehört nicht Ihnen.", type='warning')
    finally:
        db.close()
...
async def save_edited_track_details(user_id: int):
    ...
    db = db_config.SessionLocal()
    try:
        success = db_config.update_track_details(db, user_id, current_editing_track_id, new_name, labels_list) # user_id
        if success:
...
async def confirm_delete_single_track(user_id: int, track_id: int):
    db = db_config.SessionLocal()
    try:
        track_detail = db_config.get_track_details(db, user_id, track_id) # user_id
        if not track_detail:
            ui.notify(f"Track ID {track_id} nicht gefunden oder gehört nicht Ihnen.", type='warning'); return
        with ui.dialog() as conf_dialog, ui.card():
            ...
            with ui.row().classes('w-full justify-end gap-2 p-2'):
                ...
                ui.button("Löschen", on_click=lambda: delete_single_track_confirmed(user_id, track_id, conf_dialog), color='negative') # user_id
        await conf_dialog
    finally:
        db.close()
...
async def delete_single_track_confirmed(user_id: int, track_id: int, dialog_ref: ui.dialog):
    ...
    db = db_config.SessionLocal()
    try:
        deleted_track_name = db_config.delete_track_by_id_with_file(db, user_id, track_id) # user_id
...
async def confirm_delete_selected_tracks(user_id: int):
    ...
    with ui.dialog() as conf_dialog, ui.card():
        ...
        with ui.row().classes('w-full justify-end gap-2 p-2'):
            ui.button("Abbrechen", on_click=conf_dialog.close).props('flat')
            ui.button(f"{len(selected_ids_list)} Löschen", on_click=lambda: delete_multiple_tracks_confirmed(user_id, list(selected_ids_list), conf_dialog), color='negative') # user_id
    await conf_dialog
...
async def delete_multiple_tracks_confirmed(user_id: int, track_ids_to_delete: List[int], dialog_ref: ui.dialog):
    ...
    db = db_config.SessionLocal()
    try:
        num_deleted, errors = db_config.delete_multiple_tracks_with_files(db, user_id, track_ids_to_delete) # user_id
...
# Globale Einstellungen für die NiceGUI App
app.storage.secret = "MEIN_SUPER_GEHEIMER_STORAGE_KEY_UNBEDINGT_AENDERN" # Wichtig für `app.storage.user`
ui.run(title="GPX Track Manager", storage_secret=app.storage.secret, reload=False, port=8081)