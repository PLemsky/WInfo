...
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, Text, func, event, ForeignKey # ForeignKey hinzugefügt
...
DATABASE_URL = f"sqlite:///{BASE_DIR / 'tracks_users_sqlalchemy.db'}" # Neuer DB-Name
...
class UserDB(Base):
    ...
    email = Column(String, unique=True, index=True, nullable=True) # Optional, aber gut für Registrierung
...
class TrackDB(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("users.id"), nullable=False) # Fremdschlüssel zum Benutzer
    name = Column(String, index=True, nullable=False)
    original_filename = Column(String, nullable=True)
    stored_filename = Column(String, nullable=False, unique=True) # Bleibt unique
...
def create_user(db: Session, username: str, password: str, email: Optional[str] = None) -> UserDB:
...
# --- Track CRUD Operationen (angepasst für user_id) ---
...
def add_track(
    ...
    # Um sicherzustellen, dass stored_filename global unique ist, auch wenn user-spezifisch:
    # Man könnte user_id in den Namen einbauen oder einfach auf den Timestamp vertrauen.
    # Für den Moment reicht der Timestamp, da er sehr granular ist.
...
def get_track_details(db: Session, user_id: int, track_id: int) -> Optional[TrackDB]: # user_id hinzugefügt
...
def get_filtered_tracks(
    ...
    query = db.query(TrackDB).filter(TrackDB.user_id == user_id) # Filter auf user_id
    try:
        if start_date_str:
        ...
        # Im Fehlerfall alle Tracks des Benutzers zurückgeben
...
def update_track_details(db: Session, user_id: int, track_id: int, new_name: str, new_labels_list: List[str]) -> bool: # user_id hinzugefügt
...
def delete_track_by_id_with_file(db: Session, user_id: int, track_id: int) -> Optional[str]: # user_id hinzugefügt
    track = db.query(TrackDB).filter(TrackDB.id == track_id, TrackDB.user_id == user_id).first()
    if track:
...
def delete_multiple_tracks_with_files(db: Session, user_id: int, track_ids: List[int]) -> Tuple[int, List[str]]: # user_id hinzugefügt
    if not track_ids:
        return 0, []
    ...
    for track in tracks_to_delete:
        ...
        try:
            db.delete(track)
        except Exception as e_del_obj: # Fängt Fehler beim Markieren zum Löschen ab
            db.rollback() # Wichtig: Änderungen zurückrollen, wenn ein Objekt nicht gelöscht werden kann
            errors.append(f"Fehler beim Vorbereiten des Löschens für Track ID {track.id}: {e_del_obj}")
            # Sofort zurückkehren, da wir nicht fortfahren können
        ...
        for f_path in files_to_delete_paths:
            if f_path.exists():
                try:
                    ...
                    errors.append(f"Konnte Datei {f_path} nicht löschen: {e_file_del}")
        print(f"{deleted_count} Tracks für User ID {user_id} gelöscht.")
    except Exception as e_commit:
        db.rollback()
        errors.append(f"Fehler beim finalen DB-Commit für User ID {user_id}: {e_commit}")
        traceback.print_exc()
...
def get_all_unique_labels(db: Session, user_id: int) -> List[str]: # user_id hinzugefügt
...
def get_gpx_filepath(db: Session, user_id: int, track_id: int) -> Optional[Path]: # user_id hinzugefügt
    # Stelle sicher, dass der Track dem User gehört, bevor der Pfad zurückgegeben wird
    track = db.query(TrackDB).filter(TrackDB.id == track_id, TrackDB.user_id == user_id).first()
